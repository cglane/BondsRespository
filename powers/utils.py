		...
		try:
			...
			if bond.warrant_number:
				if  "\n" in bond.warrant_number:
					...
					local_warrant_number = bond.warrant_number
			if local_warrant_number:
				...
				if status:
					...
					bond.bot_error = 'Case with warrant number of: {}. Not Found.'.format(local_warrant_number)
			else:
				bond.bot_error = 'No Warrant Number'
